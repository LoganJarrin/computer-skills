...
// Create a class via the Neon Data API with the teacher's JWT. RLS WITH CHECK
// (teacher_id = auth.user_id()) guarantees a teacher can only create classes
// owned by themselves — enforced at the database level.
export async function POST(req: NextRequest) {
  ...
  try {
    for (let i = 0; i < 6; i++) {
      ...
      if ('conflict' in res) continue; // join_code collision, retry
